...
with open('/home/claude/briefing.css') as f:
    CSS = f.read()
...
<header class="toolbar" role="toolbar" aria-label="Actions principales">
  ...
  <button class="tb-btn" id="btn-mode" title="Basculer édition / aperçu">
    ...
    <span class="tb-btn-label" id="mode-label">Aperçu</span>
  </button>
  ...
  <button class="tb-btn" id="btn-print" title="Imprimer ou sauvegarder en PDF">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
    <span class="tb-btn-label">Imprimer</span>
  </button>
...
<div class="app" id="app">
  ...
  <aside class="editor" id="editor">
    ...
    <details class="ed-section" data-section="annexes">
      <summary>08 ░ ANNEXES</summary>
      <div class="ed-content">
        <div class="panel-mobile-header">08 ░ ANNEXES</div>
        <div class="ed-field"><label>Chart Aéroport 1 — Nom</label><input data-bind="annexes.chart1Name" type="text"></div>
        <div class="ed-field">
          <label>Chart Aéroport 1 — Image</label>
          <label class="ed-img-zone" data-img-bind="annexes.chart1Img">
            <input type="file" accept="image/*">
            <span class="img-text">▲ CHARGER CHART ▲</span>
          </label>
        </div>

        <div class="ed-field"><label>Chart Aéroport 2 — Nom</label><input data-bind="annexes.chart2Name" type="text"></div>
        <div class="ed-field">
          <label>Chart Aéroport 2 — Image</label>
          <label class="ed-img-zone" data-img-bind="annexes.chart2Img">
            <input type="file" accept="image/*">
            <span class="img-text">▲ CHARGER CHART ▲</span>
          </label>
        </div>
    ...
    <details class="ed-section" data-section="wing">
      <summary>09 ░ CONFIGURATION WING</summary>
      <div class="ed-content">
        <div class="panel-mobile-header">09 ░ CONFIG WING</div>
...
function renderPhaseEditor() {
  ...
  subgroupEl.addEventListener('input', () => { state.phases[i].subgroup = subgroupEl.value; schedulePreview(); });
  ...
  if (isGuest) {
    ...
    container.querySelector('#pf-guest-name').addEventListener('input', e => {
      state.phases[i].guestSquadron.name = e.target.value; schedulePreview();
    });
    container.querySelector('#pf-guest-sub').addEventListener('input', e => {
      state.phases[i].guestSquadron.subgroup = e.target.value; schedulePreview();
    });
    container.querySelector('#pf-guest-aircraft').addEventListener('input', e => {
      state.phases[i].guestSquadron.aircraft = e.target.value; schedulePreview();
    });
  }
  ...
  container.querySelector('#pf-squadron').addEventListener('change', e => {
    ...
    renderPhaseEditor(); schedulePreview();
  });

  // Aircraft selector (multi-aircraft squadrons only)
  const acSel = container.querySelector('#pf-aircraft');
  if (acSel) {
    acSel.addEventListener('change', e => { state.phases[i].aircraft = e.target.value; schedulePreview(); });
  }
...
function pageHeader(opts) {
  ...
  return `
    <header class="p-header">
      <img src="${wingConfig.wing.logo}" alt="${escapeAttr(wingConfig.wing.shortName)}">
      <div class="p-title">
      ...
      <img src="${rightLogo}" alt="${escapeAttr(rightAlt)}">
    </header>
  `;
}
...
function buildPages() {
        ...
        <div class="squadron-grid">
          ${engagedList.map(e => {
            ...
            const logoHtml = info.logo
              ? `<img src="${info.logo}" alt="${escapeAttr(info.id)}" class="squadron-logo-sm">`
  ...
  // ANNEXE — CHART 1
  if (state.annexes.chart1Img || state.annexes.chart1Name) {
    pages.push({
      body: `
        <div class="p-section">ANNEXE ░ CHART AÉROPORT</div>
        <div class="p-subsection">${escapeHtml(state.annexes.chart1Name || 'AÉROPORT')}</div>
        ${imgFrame('CHART OPS', state.annexes.chart1Img)}
      `
    });
  }
  if (state.annexes.chart2Img || (state.annexes.chart2Name && state.annexes.chart2Name !== state.annexes.chart1Name)) {
    pages.push({
      body: `
        <div class="p-section">ANNEXE ░ CHART AÉROPORT</div>
        <div class="p-subsection">${escapeHtml(state.annexes.chart2Name || 'AÉROPORT')}</div>
        ${imgFrame('CHART OPS', state.annexes.chart2Img)}
      `
    });
  }
...
function toggleMode() {
  ...
  const isPreview = app.classList.toggle('preview-only');
  document.getElementById('mode-label').textContent = isPreview ? 'Édition' : 'Aperçu';
  document.getElementById('btn-mode').classList.toggle('active', isPreview);
  // On mobile, also switch tab
  if (window.innerWidth <= MOBILE_BREAKPOINT) {
    setActiveTab(isPreview ? 'preview' : 'meta');
  }
}
...
output_path = '/mnt/user-data/outputs/DCS_World_Briefing_Generator.html'
